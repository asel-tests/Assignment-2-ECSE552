...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        xt = x[:, t, :]  # Extract input for current timestep
        h_t, c_prev, cache_t = lstm_step_forward(
            xt, h_prev, c_prev, Wx, Wh, b
        )  # Forward step
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N, T, H = dh.shape  # Extract dimensions
    x0 = cache[0][0]  # Get input x from first timestep
    D = x0.shape[1]  # Infer input dimension
    ...
    for t in reversed(range(T)):
        # Combine upstream gradient with gradient from next timestep
        dnext_h = dh[:, t, :] + dprev_h

        dnext_c = dprev_c

        # Backprop one LSTM step
